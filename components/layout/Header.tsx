          ...
          <nav className={`hidden lg:flex items-center gap-10 transition-opacity duration-300 ${isSearchOpen ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
            {navLinks.map((link) => {
              ...
              return (
                <button
                  key={link.id}
                  onClick={() => link.id === 'programs' || link.id === 'events' ? null : setCurrentPage(link.id as PageType)}
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredItems.length > 0 ? (
                filteredItems.map((item, idx) => (
                  <button 
                    key={idx} 
                    onClick={() => { setCurrentPage(item.link as PageType); setIsSearchOpen(false); }}
          ...
          <div className="p-6 flex flex-col h-full">
            ...
            <nav className="flex flex-col gap-6">
              {navLinks.map((link) => (
                <button
                  key={link.id}
                  onClick={() => { if (link.id !== 'programs' && link.id !== 'events') { setCurrentPage(link.id as PageType); setIsMobileMenuOpen(false); } }}
